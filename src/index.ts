...
function estaEnEnum(valor: string, enumObj: any): boolean {
...
yargs(hideBin(process.argv))
  ...
  marketValue: {
    ...
    if (users.getUserByName(argv.user).ownerOf.getFunko(argv.id) === undefined) {
      const FunkoToAdd = new Funko (argv.id, argv.name, argv.description, tipo as Tipo, gender as Genero, argv.franchise, argv.number, argv.exclusive, argv.specialFeatures, argv.marketValue);
    ...
    console.log(chalk.green("Funko " + argv.name + " añadido al usuario " + argv.user + " correctamente"));
    }
    else {
